...
public class OurCalendar {
            ...
            case 1: {
                ...
                while (true) {
                    ...
                    if (option.equalsIgnoreCase("Y")) {
                        ...
                        Validate.print("Wrong input try again:\n");
                    }
                }
                Validate.print("");
        ...
        do {
            ...
            for (Event event : events) {
                if (choice == 3 && event instanceof Project) {
                    ...
                    Validate.print(event.toString());
                } else if (choice == 4) {
                    return;
                }
            }

            Validate.print("Type the title of the event you want to change:");
            //Finding the title of the event:
            while (!flag) {
                title = Validate.strInput();
                searchedEvent = eventSearch(title, choice);
                if (searchedEvent == null) {
                    Validate.print("You typed wrong title. Try again.");
                } else {
                    flag = true;
                }
            }
    ...
    public void changeProjectCondition(){
        Validate.print("Please provide the name of the Project you wish to update its status");
        ...
        for (Event event : events) {
            ...
            if (eventFormat >= minTime && eventFormat < maxTime) {     //if the event is between minimum and maximum time
                if (code == 2 && (dayOfWeek.getValue() + eventDay - realDay) <= 7)  //if the event is upcoming
                    Validate.print(event.getTitle() + "\t" + event.getDateTime());
    ...
    public void reminder(){
        ...
        if (closestAppointment == null && closestDeadline == null){
            Validate.print("You dont have any reminders");
        }else if (closestAppointment == null){
            Validate.print("Your next project deadline is in " + closestDeadline.getDeadline().toString());
        }else if(closestDeadline == null){
            Validate.print("Your next appointment is in "+ closestAppointment.getDateTime().toString());
        }else {
